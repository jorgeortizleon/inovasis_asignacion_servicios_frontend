    ...
    <div class="row q-col-gutter-sm espacio-borde-table">
      <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <q-card class="text-grey-8 no-shadow" bordered>
          <q-card-section class="q-pa-none">
            <q-table ref="myTable" class="no-shadow" :rows="filteredServices" :columns="columns"
              title="Requisitos De Los Servicios" :hide-header="mode === 'grid'" :rows-per-page-options="[10000]"
              row-key="idServicio" :filter="filter" no-data-label="No hay Servicios"
              ...
              <template v-slot:body-cell-hojaRemision="props2">
                <q-td :props2="props2">
                  <div class="text-center"> <!-- Usar text-center en lugar de q-text-center -->
                    <q-badge :color="props2.value === 1 ? 'purple' : 'negative'" :label="props2.value === 1 ? 'Remisión' : '-'" />
                  </div>
                </q-td>
              </template>
              <template v-slot:body-cell-hojaServicio="props2">
                <q-td :props2="props2">
                  <div class="text-center"> <!-- Usar text-center en lugar de q-text-center -->
                    <q-badge :color="props2.value === 1 ? 'blue' : 'negative'" :label="props2.value === 1 ? 'Servicio' : '-'" />
                  </div>
                </q-td>
              </template>
              <template v-slot:body-cell-factura="props2">
                <q-td :props2="props2">
                  <div class="text-center"> <!-- Usar text-center en lugar de q-text-center -->
                    <q-badge :color="props2.value === 1 ? 'teal' : 'negative'" :label="props2.value === 1 ? 'Factura' : '-'" />
                  </div>
                </q-td>
              </template>
              <template v-slot:body-cell-empresaPoliza="props2">
                <q-td :props2="props2">
                  <div class="text-center"> <!-- Usar text-center en lugar de q-text-center -->
                    <q-badge :color="props2.value === 1 ? 'orange' : 'negative'" :label="props2.value === 1 ? 'Póliza' : '-'" />
                  </div>
                </q-td>
              </template>
            </q-table>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: 'hojaRemision', align: 'center', label: 'Hoja de Remisión', field: 'hojaRemision', sortable: true },
        { name: 'hojaServicio', align: 'center', label: ' Hoja de Servicio', field: 'hojaServicio', sortable: true },
        { name: 'factura', align: 'center', label: 'Factura', field: 'factura', sortable: true },
        { name: 'empresaPoliza', align: 'center', label: 'Empresa de Póliza', field: 'empresaPoliza', sortable: true },
        ...
        { name: 'estado', align: 'center', label: 'Estado Del Servicio', field: 'estado', sortable: false },
      ],
      items: [
        {
